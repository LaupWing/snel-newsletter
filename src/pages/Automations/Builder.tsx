...
const STEP_META: Record< string, { label: string; icon: ComponentType< any >; color: string; bg: string; border: string; handle: string } > = {
...
const NEW_STEP: Record< string, () => any > = {
...
const Line = ( { tall, color = 'bg-blue-400' }: { tall?: boolean; color?: string } ) => <div className={ `w-0.5 ${ color } ${ tall ? 'h-5' : 'h-3.5' }` } />;

/** Connection-handle dot on a node edge. */
const NodeHandle = ( { position, color }: { position: string; color: string } ) => (
...
function AddButton( { onAdd, allowCondition }: { onAdd: ( step: any ) => void; allowCondition?: boolean } ) {
...
type StepCardProps = {
    ...
    narrow?: boolean;
    path: any[];
    onInspect?: ( path: any ) => void;
};

function StepCard( { step, onChange, onRemove, campaigns, tags, emailStats, narrow, path, onInspect }: StepCardProps ) {
...
function StepList( { steps, onSteps, campaigns, tags, emailStats, allowCondition, narrow, lineColor, basePath = [], onInspect }: StepListProps ) {
    const insertAt = ( i: number, step: any ) => {
        const next = steps.slice();
        next.splice( i, 0, step );
        onSteps( next );
    };
    const updateAt = ( i: number, step: any ) => onSteps( steps.map( ( s, j ) => ( j === i ? step : s ) ) );
    const removeAt = ( i: number ) => onSteps( steps.filter( ( _, j ) => j !== i ) );
...
function ConditionBlock( { step, onChange, onRemove, campaigns, tags, emailStats, path, onInspect }: ConditionBlockProps ) {
...
export default function Builder( { automationId, onClose }: { automationId: number; onClose: () => void } ) {
    ...
    const [ inspect, setInspect ]       = useState< any >( null ); // step path, or 'trigger'
    ...
    useEffect( () => {
        ...
        api( '/tags' ).then( ( d ) => setTags( ( d || [] ).map( ( t: any ) => t.tag ) ) );
    }, [ automationId ] );

    const patch = useCallback( ( fields: any ) => {
        setAutomation( ( a: any ) => ( { ...a, ...fields } ) );
        setDirty( true );
    }, [] );
