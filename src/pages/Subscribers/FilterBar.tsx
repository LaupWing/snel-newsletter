...
const isMetric = ( field: string ) => METRIC_FIELDS.some( ( m ) => m.value === field );
const isDayWindow = ( field: string ) => field === 'opened_in_days' || field === 'clicked_in_days';

/** Sensible default operator + value when the field changes. */
function defaultsFor( field: string ): { operator: FilterOperator; value: string } {
...
export default function FilterBar( { filters, onChange, allTags }: Props ) {
    const addRow = () => {
        onChange( [ ...filters, { field: 'open_rate', ...defaultsFor( 'open_rate' ) } ] );
    };

    const updateRow = ( index: number, patch: Partial< FilterRule > ) => {
            ...
            { filters.map( ( f, i ) => {
                ...
                return (
                    <div key={ i } className="flex items-center gap-2 flex-wrap">
                        ...
                        <Select
                            value={ f.field }
                            onChange={ ( v ) => updateRow( i, { field: v as FilterField, ...defaultsFor( v ) } ) }
                        ...
                        { metric && (
                            <Select
                                value={ f.operator }
                                onChange={ ( v ) => updateRow( i, { operator: v as FilterOperator } ) }
