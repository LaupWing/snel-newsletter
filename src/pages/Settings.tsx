...
function InputField( { label, hint, type = 'text', value, onChange, placeholder, required }: InputFieldProps ) {
...
function SesSettings( { settings, setSettings }: SettingsPanelProps ) {
    const [ testEmail, setTestEmail ] = useState( '' );
    const [ testLane, setTestLane ] = useState( 'broadcast' );
    const [ testSending, setTestSending ] = useState( false );
    const [ testResult, setTestResult ] = useState< { success: boolean; message: string } | null >( null ); // { success: bool, message: string }
...
function SenderSettings( { settings, setSettings }: SettingsPanelProps ) {
    return (
        <div className="space-y-6">
            <div className="bg-white border border-gray-200 rounded-lg p-5">
                <div className="flex items-center gap-3 mb-5">
...
function LogsTab() {
    ...
    const fetchLogs = ( lvl: string, ctx: string ) => {
        setLoading( true );
