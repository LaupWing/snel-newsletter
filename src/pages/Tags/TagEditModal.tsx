...
type Props = {
    ...
    ) => Promise< { code?: string; message?: string; synced?: number | null } >;
};
...
export default function TagEditModal( { tag, onClose, onSave }: Props ) {
    const isNew = ! tag;

    const [ name, setName ]           = useState( tag?.tag || '' );
    const [ type, setType ]           = useState< 'static' | 'dynamic' >( tag?.type || 'static' );
    const [ metric, setMetric ]       = useState< MetricField >( tag?.metric || 'open_rate' );
    const [ operator, setOperator ]   = useState< MetricOperator >( tag?.operator || 'gt' );
                    ...
                    { type === 'dynamic' && (
                        <div className="bg-amber-50 border border-amber-100 rounded-lg p-4 space-y-3">
                            ...
                            <div>
                                ...
                                <select
                                    value={ metric }
                                    onChange={ ( e ) => setMetric( e.target.value as MetricField ) }
