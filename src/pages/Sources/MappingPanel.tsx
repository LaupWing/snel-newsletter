...
const STATUS_META: Record< string, { icon: ComponentType< any >; tone: string } > = {
...
function Stat( { value, label, tone = 'text-gray-900' }: StatProps ) {
	return (
...
export default function MappingPanel( { source, onBack, onSaved }: Props ) {
	...
	const [ manualTags, setManualTags ] = useState< string[] >( saved?.manual_tags || [] );
	...
	const triggers: Record< string, { name: string; status: string } > = {};
	...
	const emailOptions = source.email_fields.map( ( f: any ) => ( {
		value: f.key,
