...
function api< T = any >( path: string, opts: RequestInit = {} ): Promise< T > {
...
function CampaignTypePanel() {
    ...
    const meta = useSelect(
        ( select: any ) => select( 'core/editor' ).getEditedPostAttribute( 'meta' ),
        []
    ) || {};

    const isWorkflow = meta._snel_nl_is_workflow === '1' || meta._snel_nl_is_workflow === true;
    const setWorkflow = ( on: boolean ) => editPost( { meta: { _snel_nl_is_workflow: on ? '1' : '' } } );
...
function RecipientPanel() {
    ...
    const setFilters = ( next: FilterRule[] ) => editPost( { meta: { _snel_nl_audience_filters: next } } );
    ...
    const [ tagAudience, setTagAudience ] = useState< number | null >( null );
    useEffect( () => {
...
function SendPanel() {
    ...
    const setPreviewText = ( value: string ) => editPost( { meta: { _snel_nl_preview_text: value } } );
    ...
    const handleTestSend = () => {
        ...
        const saveBtn = document.querySelector< HTMLElement >( '.editor-post-save-draft' );
