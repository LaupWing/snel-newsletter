...
- 2026-09-23: Gmail-dip campagne 2550 onderzocht (halverwege gefilterd; infra ok). SNS-keten eindelijk live: veldnaam `SigningCertURL` + SHA256 (topic SignatureVersion=2) gefixt, delivery/delay/bounce/complaint per mail in `snel_delivery_events` (v1.9.14-16). Timezone-fix `has_pending_work`. Campagne toont Sent + cooldown-wachtenden (v1.9.17), delivery-blok per campagne (v1.9.18). Engagement-module: golven bij publiceren + dagelijkse sunset naar inactive (v1.9.19). Open: knop "Feedback aansluiten" (AWS-setup via API), Custom MAIL FROM, Postmaster aflezen.
